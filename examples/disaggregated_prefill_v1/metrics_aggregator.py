...
class MetricsAggregator:
    ...
    def aggregate_metrics(self,addtional_labels={}):
                ...
                for line in metrics_text.split('\n'):
                    ...
                    if parsed:
                        ...
                        def label_str(labels):
                            return ','.join(f'{k}="{v}"' for k, v in labels.items() if k not in ['engine'])
                        labels.update(addtional_labels)
        ...
        for (metric_name, label), values in all_metrics.items():
            if values:
                ...
                if any(x in metric_name for x in ['bucket', '_count', '_sum', '_total', 'tokens', 'requests', 'memory']):
                    # sum metrics
